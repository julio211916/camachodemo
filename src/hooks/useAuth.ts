...
export type UserRole = 'admin' | 'staff' | 'doctor' | 'patient' | 'distributor' | 'customer' | 'vendor' | null;
...
export const useAuth = () => {
  ...
  const fetchUserRole = async (userId: string) => {
    ...
    if (data && data.length > 0) {
      // Priority: admin > doctor > staff > vendor > distributor > customer > patient
      ...
      if (roles.includes('vendor')) return 'vendor' as UserRole;
